 ...
 class registra{
	...
	if($registrar == "doReg"){
		...
		echo ("<div class='alert alert-success' role='alert'>
						...
						Debes volver a loguearte para continuar!
					</div>
				</div>");
			}
			session_destroy();
			echo $this->view->render("templates/login.html");
	...
	$reg = model::registrarNuevoUsuario($nombre,$apellido,$usuario,$email,$pass,$random_hash);
		...
		if(is_array($reg)){
			...
			//print_r($reg);
				...
				// $errn = $reg["errno"]; 
			$vars["error"] = $reg["error"];
			//valores introducidos al formulario con error, para rellenar los campos y evitar que esten vacios en la carga
			$vars["pre"] = Array("nombre"=>$nombre,"apellido"=>$apellido,"usuario"=>$usuario,"email"=>$email,"pass"=>$pass);
			}else{
			$vars["result"] = $reg;
		}
